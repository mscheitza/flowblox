...
{
    public class FlowBloxMessageBox : Form
    {
        private const int CS_DROPSHADOW = 0x00020000;
        private static FlowBloxMessageBox _msgBox;
        ...
        {
            _msgBox = new FlowBloxMessageBox();
            _msgBox._lblMessage.Text = message;
            _msgBox._lblTitle.Text = "Information";
            _msgBox.Text = _msgBox._lblTitle.Text;
            InitIcon(Icons.Info);
        ...
        public static DialogResult Show(IWin32Window owner, string message, string title, Buttons buttons, Icons icon)
        {
            _msgBox = new FlowBloxMessageBox();
            _msgBox._lblMessage.Text = message;
            _msgBox._lblTitle.Text = title;
        ...
        public static DialogResult Show(IWin32Window owner, string message, string title, Buttons buttons, Icons icon, AnimateStyle style)
        {
            _msgBox = new FlowBloxMessageBox();
            _msgBox._lblMessage.Text = message;
            _msgBox._lblTitle.Text = title;
        ...
        private static Size MessageSize(string message)
        {
            ...
            {
                string withoutLineBreaks = message
                    .Replace("\r", string.Empty)
                    .Replace("\n", string.Empty);

                string[] groups = (from Match m in Regex.Matches(withoutLineBreaks, ".{1,180}") select m.Value).ToArray();
                int lines = groups.Length + 1;
                width = 700;
                height += (int)size.Height + (LineHeight * lines);
            }
    ...
    internal class AnimateMsgBox
    {
        ...
        public FlowBloxMessageBox.AnimateStyle Style;

        public AnimateMsgBox(Size formSize, FlowBloxMessageBox.AnimateStyle style)
        {
