...
namespace FlowBlox.Core.Models.FlowBlocks
{
    ...
    public class CounterFlowBlock : BaseSingleResultFlowBlock
    {
        ...
        private void ExecuteIntegerCounter(BaseRuntime runtime)
        {
            ...
            int endValue;
            if (!string.IsNullOrEmpty(this.FinalValue))
                endValue = int.Parse(FinalValue);
            else if (!string.IsNullOrEmpty(this.Format))
            {
                var resolvedFinalValue = string.Concat(Enumerable.Range(0, this.Format.Length).Select(x => "9"));
        ...
        {
            defaults.Add(new OptionElement("Counter.Alphanumeric.Range", "aäbcdefghijklmnoöpqrsßtuüvwxyz", "Legen Sie hier die Zeichen des alphanumerischen Zählers fest.", OptionElement.OptionType.Text));
